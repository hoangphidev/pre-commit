...
class PreCommitHookCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'pre-commit:check';
     ...
     * @var string
     */
    protected $description = 'Command check before commit.';
    ...
    public function handle()
    {
        ...
        if (empty($changed)) {
            $output->writeLine('[Pre-commit] Success: Nothing to check!', TextOutputColored::TYPE_OK);

            return false;
        }

        $this->info('[Pre-commit] Running PHP lint...');
        if (!$this->lint($changed)) {
            exit($this->fails());
        }

        $this->info('[Pre-commit] Checking PSR-2 Coding Standard...');
        $start = now();
        if (!$this->checkPsr2($changed)) {
            exit($this->fails());
        }
        $end = now();
        $this->output->writeln('[Pre-commit] Checked ' . count($changed) . ' file(s) in ' . $end->diffInRealSeconds($start) . ' second(s).');

        $output->writeLine('[Pre-commit] Your code is perfect, no syntax error found!', TextOutputColored::TYPE_OK);
     ...
     * @return array
     */
    protected function getChangedPhpFiles()
    {
        $changed = [];
    ...
    protected function getChangedFiles()
    {
        if (!$this->exec($cmd = 'git status --short', $output)) {
            throw new RuntimeException('[Pre-commit] Unable to run command: ' . $cmd);
        }
     ...
     * @return bool
     */
    protected function exec(string $command, &$output = null, &$status = null)
    {
    ...
    protected function lint(array $changed): bool
    {
        ...
        if (false === $output = stream_get_contents($pipes[1])) {
            throw new RuntimeException('[Pre-commit] Unable to get the lint result.');
        }
